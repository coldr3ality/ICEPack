...
for( my $ctv=0; $ctv< $#CASE_TERMINATOR; $ctv+=3 ){
 foreach my $overrunBytes(0..3){
  open(my $fh, '>',	"hiCASTt$overrunBytes$CASE_TERMINATOR[$ctv].h");
  printf $fh("#define	hiCASTt$overrunBytes%s( \$a, \$b, \$pq%s)	/*	cast [a, b] to the high side of *pq	w/ ($overrunBytes) byte[s] of overrun tolerance	*/		\\\n",
