...
#define ICEPACK( $pq, $I, $Z, $N, $dBUG_MACRO, $dBUG_MACRO_i )										$dBUG_MACRO(	$I );	\
		for(	;				$Z	>	$I;--$Z )	\
			if( Ox[ $N ] - Ox[	$Z ] >2 ){			\
				do	{	switch(	H[	$I	] ){	SwCASE_AB2ICEq_t3inc(	A[ $I ],	B[ $I ],	$pq  ); }		$dBUG_MACRO_i(	$I );	\
					}	while(	++	$I< $Z );	\
				break;							\
				}								\
		do			{	switch(	H[	$I	] ){	SwCASE_AB2ICEq_t0inc(	A[ $I ],	B[ $I ],	$pq	); }		$dBUG_MACRO_i(	$I );	\
					}	while(	++	$I< $N );

#define iCEPACK( $pq, $I, $Z, $N, $dBUG_MACRO, $dBUG_MACRO_i )										$dBUG_MACRO(	$I );	\
		for(	ix=$I;			$Z	>	ix;--$Z )	\
			if( Ox[ $N ] - Ox[	$Z ] >2 ){			\
				do	{	switch(	H[	ix	] ){	SwCASE_AB2ICEq_t3inc(	A[ ix ],  	B[ ix ],	$pq  ); }		$dBUG_MACRO_i(	ix );	\
					}	while(	++	ix< $Z );	\
				break;							\
				}								\
		do			{	switch(	H[	ix	] ){	SwCASE_AB2ICEq_t0inc(	A[ ix ],  	B[ ix ],	$pq	); }		$dBUG_MACRO_i(	ix );	\
					}	while(	++	ix< $N );
...
#define iCEpACK( $pq, $I, $Z, $N, $dBUG_MACRO, $dBUG_MACRO_i )										$dBUG_MACRO(	$I );	\
	...
	//														bytes	shift		src		src index			src out-dex	dst			dst index			dst out-dex			op symbol
		...
		#define dBUGmxEO( $X )	_print_mx( $X,	ix1, izZ );	_init_mx();
													//	if( rel_q		!=	post_q -pre_q		){ printf( "\r!	_sv_commit(): 	( rel_q = Ox[ ixH ] - O[ ixH ]	)==%d should be equal to:\n			( rel_q = post_q -pre_q	)==%d\n\n",	\
																												rel_q,						post_q -pre_q );	}
