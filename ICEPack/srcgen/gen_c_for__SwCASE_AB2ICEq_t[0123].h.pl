...
my @CASE_TERMINATOR=( 	'',				'',				'',
						'inc',			'',				'printf $fh( "	\$pq +=%3d;",					$s[A]		);',
						'incQ',			', $postQ',		'printf $fh( "	\$pq +=%3d;	\$postQ -=%3d;",	$s[A], $s[A]	);',
#	|					|				|				|
#	|					|				|				|
						);
...
for( my $ctv=0; $ctv< $#CASE_TERMINATOR; $ctv+=3 ){
 foreach my $overrunBytes(0..3){
  open(my $fh, '>',	"SwCASE_AB2ICEq_t$overrunBytes$CASE_TERMINATOR[$ctv].h");
  printf $fh(
	"/*	This file was programmatically generated.\n\t	script:\t\t$0\n\t	last modified:\t$readable_date	*/\n\n".
		"#define	SwCASE_AB2ICEq_t$overrunBytes%s( \$a, \$b, \$pq%s)	/*	cast [a, b] to the high side of *pq	w/ ($overrunBytes) byte[s] of overrun tolerance	*/		\\\n",
