...
my @CASE_TERMINATOR=( 	'',				'',				'',
						'_dec_p',			'',				'if( $s[A] >0 ){	printf $fh( "	\$pq0-=%3d;	",	$s[A]	);	}'.
														'else{		printf $fh( "				",			);	}'
...
# foreach my $overrunBytes(0..3){
  open(my $fh, '>',	"SwCASE_ICEq2XErev$CASE_TERMINATOR[$ctv].h");
  printf $fh(
	"/*	This file was programmatically generated.\n\t	script:\t\t$0\n\t	last modified:\t$readable_date	*/\n\n".
	"#define	SwCASE_ICEq2XErev%s( \$q, \$a, \$b, \$pq0, \$pq1, \$cube %s)	/*	compute [X, E] from the q-data at *pq (in reverse)		*/		\\\n",
  ...
  print $fh("\\\n/*	part 1 of 4:	A disabled;	B disabled	(neither)	*/	\\\n"	);
			...
			for( $q[B]=1;		$q[B]< 9; ++$q[B] ){
				for( $q[A]=1;	$q[A]< 9; ++$q[A] ){	#	q( A ): 1..8	q( B ): 0
					$qs=		( ( $q[B] -1) <<3)|	($q[A]-1);
  printf $fh("case 0x%02X:	/* %2d, %-2d  */	\$X =\$E -%d;".								"	$T	\$E =\$X -%d;$T",										$qs, 0, 0,				$q[B] -1,					$q[A] -1	);
...
#  printf $fh("case 0x%02X:	/* %2d, %-2d  */	$ABCD[0]= *( ($CAST[ $q[A]]*) \$pq0		)".	"%-18s	\t	$ABCD[1]= %d;$T",										$qs, $q[A], 0,			$BS[$q[A] ],				$q[B] -1	);
  printf $fh("case 0x%02X:	/* %2d, %-2d  */	\$X =\$E -%d;".								"	$T	\$E =\$X -*( ($CAST[ $q[A]]*) \$pq0		)".	"%-18s	",	$qs, 0, $q[A],	 			$q[B]-1,				$BS[$q[A] ]	);
}else{
#  printf $fh("case 0x%02X:	/* %2d, %-2d  */	$ABCD[0]= *( ($CAST[ $q[A]]*) (\$pq0 %+d )\t)".	"%-18s	\t	$ABCD[1]= %d;$T",										$qs, $q[A], 0,	$o[A],	$BS[$q[A] ],				$q[B] -1	);
  printf $fh("case 0x%02X:	/* %2d, %-2d  */	\$X =\$E -%d;".								"	$T	\$E =\$X -*( ($CAST[ $q[A]]*) (\$pq0 %+2d)\t)".	"%-18s	",	$qs, 0, $q[A],	 			$q[B]-1,		$o[A],	$BS[$q[A] ]	);
	}
...
#  printf $fh("case 0x%02X:	/* %2d, %-2d  */	$ABCD[0]= %d;".								"	$T	$ABCD[1]= *( ($CAST[ $q[B]]*) \$pq0		)".	"%-18s	",	$qs, 0, $q[B],	 			$q[A]-1,				$BS[$q[B] ]	);
  printf $fh("case 0x%02X:	/* %2d, %-2d  */	\$X =\$E -*( ($CAST[ $q[B]]*) \$pq0		)".	"%-18s	\t	\$E =\$X -%d;$T",										$qs, $q[B], 0,			$BS[$q[B] ],				$q[A] -1	);
}else{
#  printf $fh("case 0x%02X:	/* %2d, %-2d  */	$ABCD[0]= %d;".								"	$T	$ABCD[1]= *( ($CAST[ $q[B]]*) (\$pq0 %+2d)\t)".	"%-18s	",	$qs, 0, $q[B],	 			$q[A]-1,		$o[B],	$BS[$q[B] ]	);
  printf $fh("case 0x%02X:	/* %2d, %-2d  */	\$X =\$E -*( ($CAST[ $q[B]]*) (\$pq0 %+d )\t)".	"%-18s	\t	\$E =\$X -%d;$T",										$qs, $q[B], 0,	$o[B],	$BS[$q[B] ],				$q[A] -1	);
	}
					eval( $CASE_TERMINATOR[ $ctv +2 ] );
					print $fh( "	break;	\\\n");
				}	}
...
if(		$o[B ]==0){
  if(		$o[A ]==0){
    printf $fh("case 0x%02X:	/* %2d, %-2d  */	\$X =\$E -*( ($CAST[ $q[B]]*) \$pq0		)".	"%-18s	\t	\$E =\$X -*( ($CAST[ $q[A]]*) \$pq0		)".	"%-18s	",	$qs, $q[A], $q[B],			$BS[$q[B] ],			$BS[$q[A] ]	);
  }else{
    printf $fh("case 0x%02X:	/* %2d, %-2d  */	\$X =\$E -*( ($CAST[ $q[B]]*) \$pq0		)".	"%-18s	\t	\$E =\$X -*( ($CAST[ $q[A]]*) (\$pq0 %+2d)\t)".	"%-18s	",	$qs, $q[A], $q[B],			$BS[$q[B] ],	$o[A],	$BS[$q[A] ]	);
  }
}elsif(	$o[A ]==0){
    printf $fh("case 0x%02X:	/* %2d, %-2d  */	\$X =\$E -*( ($CAST[ $q[B]]*) (\$pq0 %+2d)\t)".	"%-18s	\t	\$E =\$X -*( ($CAST[ $q[A]]*) \$pq0		)".	"%-18s	",	$qs, $q[A], $q[B],	$o[B],	$BS[$q[B] ],			$BS[$q[A ] ]	);
}else{
    printf $fh("case 0x%02X:	/* %2d, %-2d  */	\$X =\$E -*( ($CAST[ $q[B]]*) (\$pq0 %+2d)\t)".	"%-18s	\t	\$E =\$X -*( ($CAST[ $q[A]]*) (\$pq0 %+2d)\t)".	"%-18s	",	$qs, $q[A], $q[B],	$o[B],	$BS[$q[B] ],	$o[A],	$BS[$q[A] ]	);
	}
					eval( $CASE_TERMINATOR[ $ctv +2 ] );
					print $fh( "	break;	\\\n");
				}	}
