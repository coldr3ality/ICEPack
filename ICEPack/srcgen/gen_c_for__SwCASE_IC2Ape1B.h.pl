...
my @CASE_TERMINATOR=(	'_incxQ',			', $u, $v',    		'if( $s[A] >0 ){	printf $fh( " \$q+=%3d;	",			$s[A]	);	'.
														'			printf $fh( " \$v =\$u +\$q;	"				);	}'.
														'else{		printf $fh( " \$v =\$u;		",				);	}',
						);
...
# foreach my $overrunBytes(0..3){
  open(my $fh, '>',	"SwCASE_IC2Ape1B$CASE_TERMINATOR[$ctv].h");
  printf $fh(
	"/*	This file was programmatically generated.\n\t	script:\t\t$0\n\t	last modified:\t$readable_date	*/\n\n".
	"#define	SwCASE_IC2Ape1B%s(		\$q, \$a, \$b, \$pq  %s)	/*	Expand [a, b] from the q-data at *pq, adding (+1) to a.		*/		\\\n",
